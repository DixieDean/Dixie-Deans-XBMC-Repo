...
class OSD(xbmcgui.WindowXMLDialog):
    ...
    def __new__(cls, channel='', epgMode=False, osdMode=False):
        ...
        # xml = 'script-tvguide-changer.xml'
        # return super(OSD, cls).__new__(cls, xml, dixie.getSkinPath(xml))
    ...
    def updateLabels(self):
        ...
        channel = self.getChannel(self.channel)
        offset  = datetime.timedelta(minutes=channel.offset)

   
        if self.now:
            when = self.now.startDate + offset
            xbmcgui.Window(10000).setProperty('OTT_NOW_TITLE',  self.now.title)
            xbmcgui.Window(10000).setProperty('OTT_NOW_TIME',   dixie.formatTime(when))
            xbmcgui.Window(10000).setProperty('OTT_PROG_DESC',  self.now.description)
            self.getControl(OSD_NOWFOCUS).setVisible(True)

        if self.other:
            when = self.other.startDate + offset
            ...
            xbmcgui.Window(10000).setProperty('OTT_NEXT_TIME',   dixie.formatTime(when))

        elif self.next:
            when = self.next.startDate + offset
        ...
        xbmcgui.Window(10000).setProperty('OTT_PROG_DESC',  description)
    ...
    def playChannel(self, _channel):
        ...
        if streamUrl:
            if not DSF:
                program = self.infoService.getCurrentProgram(channel)
                if 'lliq' in streamUrl:
                    streamUrl += "/%s" % (urllib.quote(program))
                    return streamUrl

                if '.meta' in streamUrl:
                    streamUrl += "/%s/en" % (urllib.quote(program))
                    return streamUrl
                ...
                while streamUrl == 'addMore':
                    ...
                    if d.stream is not None:
                        self.setCustomStreamUrl(channel, d.stream, d.label)
